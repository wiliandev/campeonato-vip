...
if(isset($_POST["action"])){
  ...
  if($_POST["action"] == "user_fetch"){
    ...
    while ($row = mysqli_fetch_assoc($usersRecords)){
      ...
      {
        $status = '<label class="badge badge-danger">Inativo</label>';
      }
  ...
  if($_POST["action"] == "single_fetch"){
    ...
    $id = $_POST['user_id'];      
    $sql = "SELECT id, fullname, username, email, link, gender, status FROM tbl_users WHERE id = '$id'";
    $result = mysqli_query($conn, $sql);
